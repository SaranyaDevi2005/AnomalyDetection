...
def play_alarm():
    ...
    # Wait until the alarm_stop_event is set
    while not alarm_stop_event.wait(timeout=0.5):
        pass

    pygame.mixer.music.stop()  # Stop the music when event is set
    alarm_playing = False
    print("Alarm stopped.")
...
def stop_alarm():
    ...
    alarm_stop_event.set()  # Signal the alarm thread to stop immediately
    monitoring_active = False  # Stop monitoring
    return render_template('index.html', prediction_text=latest_prediction, details=latest_details)
